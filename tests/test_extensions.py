...
def test_extension_registries_load_and_cross_reference(repo_root):
    ...
    assert len(extensions.host_profiles) >= 4
    assert len(extensions.capabilities) >= 8
    assert len(extensions.expression_profiles) >= 4
    assert len(extensions.artifact_classes) >= 4
    assert len(extensions.actualization_protocols) >= 6
    assert len(extensions.workflow_recipes) >= 7
    ...
    host_profile_ids = {profile.id for profile in extensions.host_profiles}
    ...
    assert "host_claude_code" in host_profile_ids
    assert "host_claude_desktop" in host_profile_ids
    assert "host_hermes_remote" in host_profile_ids
    ...
    assert "wfr_comparison_result_bundle_technical" in workflow_recipe_ids
    ...
    assert "actx_comparison_result_bundle_packaging" in {protocol.id for protocol in extensions.actualization_protocols}
    ...
    for host_profile in extensions.host_profiles:
        assert set(host_profile.capability_ids).issubset(capability_ids)
